...
long Score=0,Heighest=0; // Show Score

int  i,j;                // Loops
int  Height=24,Width=60; // Height & Width Of Window
char Choice;             // Geting Choice
int  Heart=3;            // Display Total Hearts
int  Game_Started=0;     // Game Is Started Or Not

FILE *fp; // To Store Heigh Score In "Score.txt" File
...
void Game_Start()
{
	...
	textcolor(LIGHTRED);
	gotoxy(20,2); cprintf("Welcome To Our Game");
	gotoxy(24,5); cprintf("Developed By");
	gotoxy(27,10); cprintf("Version");
	...
	gotoxy(24,7);  cprintf("Shiyal Pradip");
	gotoxy(28,12); cprintf("3.6.9");

	textcolor(LIGHTCYAN+BLINK);
	gotoxy(20,14); cprintf("Press Any Key For Play");
	...
	textcolor(YELLOW);
	gotoxy(5,16); cprintf("Loading...");

	textcolor(WHITE);
	gotoxy(5,17);

	// 220 Is a ASCII Value Of "Ü" Symbol
	for(i=0; i<48; i++) cprintf("%c",220);
